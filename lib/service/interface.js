 ...
 * @param {boolean} [options.disabled=false] - Whether this property will be advertised on the bus.
 */
function property (options) {
   ...
   * @param {ClassFieldDecoratorContext | ClassGetterDecoratorContext | ClassSetterDecoratorContext} context
   */
  function propertyDecorator (originalProp, context) {
    if (context.kind !== 'field' && context.kind !== 'getter' && context.kind !== 'setter') {
    ...
    context.addInitializer(function () {
      ...
      this.$properties[context.name] = options;
    });
...
function method (options) {
  ...
  function methodDecorator (originalFn, context) {
    ...
    // Save a reference to the original method for the bus to execute in handlers::handleMessage
    options.fn = originalFn;

    // Register an initializer that will attach the metadata once the class is fully defined.
    context.addInitializer(function () {
      if (!this.$methods) {
        this.$methods = {};
      }
      this.$methods[context.name] = options;
    });

    // Return the original method (or a replacement if needed).
    return originalFn;
  }
...
function signal (options) {
  ...
  function signalDecorator (originalFn, context) {
    ...
    context.addInitializer(function () {
      ...
      this.$signals[context.name] = options;
    });
    return newFn;
  }
